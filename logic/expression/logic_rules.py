...
from sympy import And, Implies, Not, Or, Symbol, false, sift, true
from sympy.logic.boolalg import BooleanFalse, BooleanFunction, BooleanTrue

from .expr_tree import convert_to_not, remove_double_not, revert_not
...
def double_negative(_expr: BooleanFunction) -> list:
    ...
    results = list(map(lambda _ex: ("Phủ định của phủ định",
                                    _ex, _ex.args[0].args[0]), not_expr))
    return results
...
def constant_negative(_expr: BooleanFunction) -> list:
    ...
    not_expr = _expr.atoms(Not)
    not_expr = set(filter(
        lambda _ex: _ex.args[0].func is BooleanFalse or _ex.args[0].func is BooleanTrue, not_expr))
...
def negation_law(_expr: BooleanFunction) -> list:
    ...
    def apply(ex, _remove_ex, _remove_not_ex):
        ...
        new_args = set(ex.args).difference(
            _remove_ex).difference(_remove_not_ex)
    ...
    for _ex in parent_expr:
        not_ex, other_ex = sift(
            _ex.args, lambda _x: _x.func is Not, binary=True)
        ...
        double_not_ex, single_not_ex = sift(
            not_ex, lambda _x: _x.args[0].func is Not, binary=True)
...
def domination_and_identity(_expr: BooleanFunction) -> list:
    ...
    for _ex in parent_expr:
        true_args, new_args = sift(
            _ex.args, lambda _x: _x.func is BooleanTrue, binary=True)
        ...
        false_args, new_args = sift(
            _ex.args, lambda _x: _x.func is BooleanFalse, binary=True)
...
def absorption(_expr: BooleanFunction) -> list:
    ...
    for _ex in parent_expr:
        ...
        inner_ex, other_args = sift(
            _ex.args, lambda _x: _x.func is op_func, binary=True)
        ...
        if op_ex.__len__() < 1:
            ...
            while length < other_args.__len__():
                ...
                for sub_expr in itertools.combinations(other_args, length):
                    ...
                    sub_args = list(
                        map(lambda _x: _ex.func(left_args, _x), right_args))
                    new_distribution_expr = op_func(*sub_args)

                    _, redundant_args = sift(
                        other_args, lambda _x: _x in sub_expr, binary=True)
            ...
            for _ex1, _ex2 in itertools.combinations(op_ex, 2):
                ...
                if left_args.__len__() <= 3 or right_args.__len__() <= 3:
                    sub_args = [_ex.func(lhs, rhs) for lhs, rhs in itertools.product(
                        left_args, right_args)]
                    _, other_inner = sift(
                        op_ex, lambda _x: _x == _ex1 or _x == _ex2, binary=True)
                    new_distribution_expr = op_func(*sub_args)
                    if other_args.__len__() > 0 or other_inner.__len__() > 0:
                        new_args = [new_distribution_expr,
                                    *other_args, *other_inner]
                        new_expr = _ex.func(*new_args)
                    else:
                        new_expr = new_distribution_expr
                    results.append(("Luật phân phối", _ex, new_expr))
                ...
                for sub_expr in itertools.combinations(inner_ex, length):
                    ...
                    if dup_args.__len__() > 0 and dup_args not in sub_args:
                        ...
                        if inner_ex.__len__() > length:
                            _, new_inner = sift(
                                inner_ex, lambda _x: _x in sub_expr, binary=True)
...
def absorption_and_distribution(_expr: BooleanFunction) -> list:
    ...
    for _ex in parent_expr:
        ...
        not_other_expr_set = set(
            map(lambda _x: convert_to_not(_x), other_args))
        for in_ex in inner_ex:
            ...
            _, other_inner_expr = sift(
                inner_ex, lambda _x: _x == in_ex, binary=True)

            if dup_not_args.__len__() > 0:
                # p | (~p & q)
                _, new_inner_args = sift(
                    in_args, lambda _x: _x in dup_not_args, binary=True)
                if new_inner_args.__len__() == 0:
                    new_inner_expr = true if _ex.func is Or else false
                    _, new_other_args = sift(other_args, lambda _x: convert_to_not(
                        _x) in dup_not_args, binary=True)
                    new_args = [new_inner_expr, *
                                new_other_args, *other_inner_expr]
                    new_ex = _ex.func(*new_args)
                ...
                for not_expr in not_other_expr_set:
                    ...
                    if dup_not_args.__len__() == not_expr_args.__len__():
                        _, new_inner_args = sift(
                            in_ex.args, lambda _x: _x in dup_not_args, binary=True)
                        if new_inner_args.__len__() == 0:
                            continue
                        new_inner_expr = op_func(*new_inner_args)
                        new_args = [new_inner_expr, *
                                    other_args, *other_inner_expr]
                        new_ex = _ex.func(*new_args)
                        results.append(
        ...
        if inner_ex.__len__() > 1:
            for _ex1, _ex2 in itertools.combinations(inner_ex, 2):
                ...
                if ex1_not in _ex2.args:
                    ...
                    new_inner_expr = op_func(*new_inner_args)
                    _, other_inner = sift(
                        inner_ex, lambda _x: _x == _ex1 or _x == _ex2, binary=True)
                    new_args = [new_inner_expr, _ex1,
                                *other_args, *other_inner]
                    new_ex = _ex.func(*new_args)
                    results.append(("Luật phân phối và hấp thụ", _ex, new_ex))
...
def de_morgan_reduce(_expr: BooleanFunction) -> list:
    ...
    for _ex in parent_expr:
        ...
        not_args, other_args = sift(
            _ex.args, lambda _arg: _arg.func is Not, binary=True)
